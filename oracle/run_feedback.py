...
# File paths
MASTERFILE_PATH = Path(__file__).parent.parent / "MASTERFILE.csv"
...
class RulesBasedOracle:
    ...
    def _load_masterfile(self):
        """Load masterfile with customer data"""
        log_message("Loading MASTERFILE...")
        self.masterfile_df = pd.read_csv(MASTERFILE_PATH)
        self.masterfile_df.set_index('customer_id', inplace=True)
        log_message(f"✓ MASTERFILE loaded: {len(self.masterfile_df)} customers\n")
