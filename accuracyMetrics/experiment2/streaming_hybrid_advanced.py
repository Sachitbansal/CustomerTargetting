...
class StreamingHybridAdvanced:
    def __init__(self, num_dim, cat_dims, kMax, 
                 significance_level=0.001,  # CHANGE 1: Lower alpha = Higher threshold (99.9% coverage)
                 learning_rate=0.02, neg_learning_rate=0.05,
                 max_idle_iterations=1000,  # CHANGE 2: Keep clusters alive longer (was 500)
                 fn_buffer_size=25,         # CHANGE 3: React faster to new patterns (was 50)
                 fn_buffer_kMax=3,
                 merge_dist_threshold=0.6, 
                 weight_prune_threshold=0.001, # CHANGE 4: Harder to kill small clusters
                 radius_multiplier=1.2):       # CHANGE 5: Explicitly widen the net
        ...
        # Calculate base Chi2 threshold
        base_threshold = chi2.ppf(1 - significance_level, df=self.num_dim)
        
        # Apply multiplier to make the boundary very permissive
        self.chi2_threshold = base_threshold * radius_multiplier
        
        # Threshold for resetting idle counter (if point is somewhat close)
    ...
    def fit_batch(self, X_num, X_cat):
        ...
        # Added reg_covar to batch fit as well for consistency
        gmm = GaussianMixture(n_components=k_init, covariance_type='full', reg_covar=1e-4, random_state=42)
        ...
        for k in range(self.n_components):
            ...
            for i, dim in enumerate(self.cat_dims):
                if len(sub_c) > 0:
                    ...
                    # Smoothed Laplace (0.5) to handle unseen categories better
    ...
    def predict_score(self, x_num, x_cat):
        ...
        for k in range(self.n_components):
            try:
                # CHANGE 6: Increased regularization (1e-5 -> 1e-4)
                # This makes clusters slightly "fatter", catching edge cases
        ...
        # The threshold here is now significantly higher due to Init changes
        return (best_m < self.chi2_threshold) and (best_s > -150), best_k, best_s, all_m
    ...
    def _proc_buf(self):
        ...
        # CHANGE 7: Try forcing at least 1 component if buffer fills to avoid empty returns
        for k in range(1, self.fn_buffer_kMax+1):
            if len(Xn)<k: break
            # Added reg_covar here too
            g = GaussianMixture(n_components=k, reg_covar=1e-4, random_state=0).fit(Xn)
            if g.bic(Xn) < best_b: best_b, best_g = g.bic(Xn), g
            
        if best_g:
            lbs = best_g.predict(Xn)
            # Give new clusters slightly more initial weight to survive pruning
            wt = self.omega * (len(Xn)/20.0) 
            self.weights *= (1-wt)
