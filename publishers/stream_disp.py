...
def stream_disp(batch_size=10, delay=5):
    """
    Stream rows from stream_tables/disp.csv to present_tables/disp.csv
    ...
    """
    stream_file = os.path.join(STREAM_PATH, DISP_FILE)
    present_file = os.path.join(PRESENT_PATH, DISP_FILE)
    ...
    try:
        ...
        if rows_to_add == 0:
            print(f"[DISP] No rows available in stream")
        ...
        print(f"[DISP] Added {rows_to_add} rows | Total present: {len(updated_present_df)} | Remaining stream: {len(remaining_stream_df)}")
        ...
        print(f"[DISP] Error during streaming: {str(e)}")
        return {"error": str(e), "rows_added": 0}


def stream_disp_continuous(batch_size=10, delay=5, max_iterations=None):
    """
    ...
    print(f"\n{'='*60}")
    print(f"Starting continuous streaming for DISP")
    ...
    while True:
        ...
        if max_iterations and iteration > max_iterations:
            print(f"\n[DISP] Reached maximum iterations ({max_iterations})")
            break

        result = stream_disp(batch_size=batch_size, delay=delay)

        if "error" in result:
            print(f"[DISP] Stopping due to error")
            break

        total_rows_added += result["rows_added"]

        if result["remaining_stream"] == 0:
            print(f"\n[DISP] Stream is empty. Stopping.")
            break

    print(f"\n{'='*60}")
    print(f"DISP Streaming Complete")
...
if __name__ == "__main__":
    ...
    stream_disp_continuous(batch_size=10, delay=3)
