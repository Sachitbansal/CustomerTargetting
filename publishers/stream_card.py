...
def stream_card(batch_size=8, delay=5):
    """
    Stream rows from stream_tables/card.csv to present_tables/card.csv
    ...
    """
    stream_file = os.path.join(STREAM_PATH, CARD_FILE)
    present_file = os.path.join(PRESENT_PATH, CARD_FILE)
    ...
    try:
        ...
        if rows_to_add == 0:
            print(f"[CARD] No rows available in stream")
        ...
        print(f"[CARD] Added {rows_to_add} rows | Total present: {len(updated_present_df)} | Remaining stream: {len(remaining_stream_df)}")
        ...
        print(f"[CARD] Error during streaming: {str(e)}")
        return {"error": str(e), "rows_added": 0}


def stream_card_continuous(batch_size=8, delay=5, max_iterations=None):
    """
    ...
    print(f"\n{'='*60}")
    print(f"Starting continuous streaming for CARD")
    ...
    while True:
        ...
        if max_iterations and iteration > max_iterations:
            print(f"\n[CARD] Reached maximum iterations ({max_iterations})")
            break

        result = stream_card(batch_size=batch_size, delay=delay)

        if "error" in result:
            print(f"[CARD] Stopping due to error")
            break

        total_rows_added += result["rows_added"]

        if result["remaining_stream"] == 0:
            print(f"\n[CARD] Stream is empty. Stopping.")
            break

    print(f"\n{'='*60}")
    print(f"CARD Streaming Complete")
...
if __name__ == "__main__":
    ...
    stream_card_continuous(batch_size=8, delay=3)
