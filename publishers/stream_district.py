...
def stream_district(batch_size=5, delay=5):
    """
    Stream rows from stream_tables/district.csv to present_tables/district.csv
    ...
    """
    stream_file = os.path.join(STREAM_PATH, DISTRICT_FILE)
    present_file = os.path.join(PRESENT_PATH, DISTRICT_FILE)
    ...
    try:
        ...
        if rows_to_add == 0:
            print(f"[DISTRICT] No rows available in stream")
        ...
        print(f"[DISTRICT] Added {rows_to_add} rows | Total present: {len(updated_present_df)} | Remaining stream: {len(remaining_stream_df)}")
        ...
        print(f"[DISTRICT] Error during streaming: {str(e)}")
        return {"error": str(e), "rows_added": 0}


def stream_district_continuous(batch_size=5, delay=5, max_iterations=None):
    """
    ...
    print(f"\n{'='*60}")
    print(f"Starting continuous streaming for DISTRICT")
    ...
    while True:
        ...
        if max_iterations and iteration > max_iterations:
            print(f"\n[DISTRICT] Reached maximum iterations ({max_iterations})")
            break

        result = stream_district(batch_size=batch_size, delay=delay)

        if "error" in result:
            print(f"[DISTRICT] Stopping due to error")
            break

        total_rows_added += result["rows_added"]

        if result["remaining_stream"] == 0:
            print(f"\n[DISTRICT] Stream is empty. Stopping.")
            break

    print(f"\n{'='*60}")
    print(f"DISTRICT Streaming Complete")
...
if __name__ == "__main__":
    ...
    stream_district_continuous(batch_size=5, delay=3)
