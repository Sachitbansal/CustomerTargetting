...
LOAN_LABELS_FILE = "loan_labels.csv"

def stream_loan_labels(batch_size=5, delay=5):
    """
    Stream rows from stream_tables/loan_labels.csv to present_tables/loan_labels.csv
    ...
    """
    stream_file = os.path.join(STREAM_PATH, LOAN_LABELS_FILE)
    present_file = os.path.join(PRESENT_PATH, LOAN_LABELS_FILE)
    ...
    try:
        ...
        if rows_to_add == 0:
            print(f"[LOAN_LABELS] No rows available in stream")
        ...
        print(f"[LOAN_LABELS] Added {rows_to_add} rows | Total present: {len(updated_present_df)} | Remaining stream: {len(remaining_stream_df)}")
        ...
        print(f"[LOAN_LABELS] Error during streaming: {str(e)}")
        return {"error": str(e), "rows_added": 0}


def stream_loan_labels_continuous(batch_size=5, delay=5, max_iterations=None):
    """
    ...
    print(f"\n{'='*60}")
    print(f"Starting continuous streaming for LOAN_LABELS")
    ...
    while True:
        ...
        if max_iterations and iteration > max_iterations:
            print(f"\n[LOAN_LABELS] Reached maximum iterations ({max_iterations})")
            break

        result = stream_loan_labels(batch_size=batch_size, delay=delay)

        if "error" in result:
            print(f"[LOAN_LABELS] Stopping due to error")
            break

        total_rows_added += result["rows_added"]

        if result["remaining_stream"] == 0:
            print(f"\n[LOAN_LABELS] Stream is empty. Stopping.")
            break

    print(f"\n{'='*60}")
    print(f"LOAN_LABELS Streaming Complete")
...
if __name__ == "__main__":
    # Example usage: Stream in batches of 5 rows with 3 second delay
    stream_loan_labels_continuous(batch_size=5, delay=3)
