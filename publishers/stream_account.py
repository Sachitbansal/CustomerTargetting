...
def stream_account(batch_size=10, delay=5):
    """
    Stream rows from stream_tables/account.csv to present_tables/account.csv
    ...
    """
    stream_file = os.path.join(STREAM_PATH, ACCOUNT_FILE)
    present_file = os.path.join(PRESENT_PATH, ACCOUNT_FILE)
    ...
    try:
        ...
        if rows_to_add == 0:
            print(f"[ACCOUNT] No rows available in stream")
        ...
        print(f"[ACCOUNT] Added {rows_to_add} rows | Total present: {len(updated_present_df)} | Remaining stream: {len(remaining_stream_df)}")
        ...
        print(f"[ACCOUNT] Error during streaming: {str(e)}")
        return {"error": str(e), "rows_added": 0}


def stream_account_continuous(batch_size=10, delay=5, max_iterations=None):
    """
    ...
    print(f"\n{'='*60}")
    print(f"Starting continuous streaming for ACCOUNT")
    ...
    while True:
        ...
        if max_iterations and iteration > max_iterations:
            print(f"\n[ACCOUNT] Reached maximum iterations ({max_iterations})")
            break

        result = stream_account(batch_size=batch_size, delay=delay)

        if "error" in result:
            print(f"[ACCOUNT] Stopping due to error")
            break

        total_rows_added += result["rows_added"]

        if result["remaining_stream"] == 0:
            print(f"\n[ACCOUNT] Stream is empty. Stopping.")
            break

    print(f"\n{'='*60}")
    print(f"ACCOUNT Streaming Complete")
...
if __name__ == "__main__":
    ...
    stream_account_continuous(batch_size=10, delay=3)
