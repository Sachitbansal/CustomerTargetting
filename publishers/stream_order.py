...
def stream_order(batch_size=10, delay=5):
    """
    Stream rows from stream_tables/order.csv to present_tables/order.csv
    ...
    """
    stream_file = os.path.join(STREAM_PATH, ORDER_FILE)
    present_file = os.path.join(PRESENT_PATH, ORDER_FILE)
    ...
    try:
        ...
        if rows_to_add == 0:
            print(f"[ORDER] No rows available in stream")
        ...
        print(f"[ORDER] Added {rows_to_add} rows | Total present: {len(updated_present_df)} | Remaining stream: {len(remaining_stream_df)}")
        ...
        print(f"[ORDER] Error during streaming: {str(e)}")
        return {"error": str(e), "rows_added": 0}


def stream_order_continuous(batch_size=10, delay=5, max_iterations=None):
    """
    ...
    print(f"\n{'='*60}")
    print(f"Starting continuous streaming for ORDER")
    ...
    while True:
        ...
        if max_iterations and iteration > max_iterations:
            print(f"\n[ORDER] Reached maximum iterations ({max_iterations})")
            break

        result = stream_order(batch_size=batch_size, delay=delay)

        if "error" in result:
            print(f"[ORDER] Stopping due to error")
            break

        total_rows_added += result["rows_added"]

        if result["remaining_stream"] == 0:
            print(f"\n[ORDER] Stream is empty. Stopping.")
            break

    print(f"\n{'='*60}")
    print(f"ORDER Streaming Complete")
...
if __name__ == "__main__":
    ...
    stream_order_continuous(batch_size=10, delay=3)
